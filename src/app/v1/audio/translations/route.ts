...
export async function POST(req: NextRequest) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      const upstreamResponse = await fetch('https://api.groq.com/openai/v1/audio/translations', {
        method: 'POST',
        headers: {
  ...
  return NextResponse.json(
    { error: { message: 'Translation processing failed: ' + (lastError?.message || 'Cluster error'), type: 'api_error' } },
